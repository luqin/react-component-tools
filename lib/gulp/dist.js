...
exports['default'] = function (gulp, config) {
  ...
  function buildScss(src, dest, title) {
    return gulp.src(src).pipe(_gulpSourcemaps2['default'].init()).pipe((0, _gulpSass2['default'])()).pipe((0, _gulpRename2['default'])(config.component.name + '.css')).pipe(_gulpSourcemaps2['default'].write('./')).on('error', console.error.bind(console)) // eslint-disable-line no-console
